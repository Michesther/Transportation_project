        ...
        if ($result->num_rows > 0) {
            ...
            if ($profilePicture['error'] == UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($profilePicture["tmp_name"], $targetFile)) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO users (firstname, lastname, email, password, otp, profile_picture) VALUES (?, ?, ?, ?, ?, ?)");
                    if (!$stmt) {
                        die('Error: ' . $conn->error);  // Error if the preparation failed
                    }
                    $stmt->bind_param("ssssss", $firstname, $lastname, $email, $password, $otp, $targetFile);
...
<body>
    <div class="form-container">
        ...
        <form action="" method="POST" enctype="multipart/form-data"> <!-- Added enctype for file upload -->
            ...
            <input type="file" name="profile_picture" accept="image/*" required> <!-- Profile picture input -->
